...
def parse_single_subdir(dhdl_file, offset_fix=0, debug=False, skip_time=0.0):
    ...
    (current_lambda,
     ...
     time_arr,
     data_arr,
     legends_map) = parse_dhdl_xvg_current_and_targets(dhdl_file,
                                                       debug=debug,
                                                       skip_time=skip_time)
    ...
    next_lambda, col_index = find_next_lambda_and_column(current_lambda, targets_map,
                                                         data_arr.shape,
...
def gather_dU_per_subdir(base_dir, offset_fix=0, debug=False, skip_time=0.0):
    ...
    for (sdir_idx, dhdl_file) in subdirs:
        cLam, nLam, deltaU, col_idx = parse_single_subdir(dhdl_file,
                                                          offset_fix=offset_fix,
                                                          debug=debug,
                                                          skip_time=skip_time)
        if cLam is None:
        ...
        if nLam is None or deltaU is None:
            print(f"[INFO] No next-lambda found (or no data left) for subdir with λ={cLam:.4f} in {dhdl_file}. Skipping.")
            continue
...
def plot_dU_distributions_consecutive_grid(base_dir1,
                                           base_dir2=None,
                                           offset_fix=0,
                                           debug=False,
                                           normalize=False,
    ...
    if base_dir2 is not None:
        bwd_list = gather_dU_per_subdir(base_dir2, offset_fix=offset_fix,
                                        debug=debug, skip_time=skip_time)
    ...
    if n_plots == 0:
        print("[WARNING] No transitions found (or no data after skip_time) in the provided directories.")
        return
    ...
    if base_dir2 is None:
        if normalize:
            ...
            ybox = TextArea(r"$P_{fwd}(\Delta U)$",
                            textprops=dict(color="k", size=30, rotation=90,
                                           ha='center', va='bottom'))
    else:
        if normalize:
