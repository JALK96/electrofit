    ...
    for group, patterns in GROUPS.items():
        for pat in patterns:
            npy_path_sub = os.path.join(
                root_dir, f"IP_{pat}",
                "analyze_final_sim", "NaP_coordination",
                "NaP_coordination_bool.npy"
            )
            npy_path_flat = os.path.join(
                root_dir, f"IP_{pat}",
                "analyze_final_sim", "NaP_coordination_bool.npy"
            )
    ...
    for group, patterns in GROUPS.items():
        for pat in patterns:
            ...
            for suf in suffixes:
                ...
                fpath = os.path.join(
                    root_dir, f"IP_{pat}",
                    "analyze_final_sim", "NaP_dist_count", fname
                )
    ...
    for group, patterns in GROUPS.items():
        ...
        for pat in patterns:
            ...
            for suf in suffixes:
                fpath = os.path.join(
                    root_dir, f"IP_{pat}",
                    "analyze_final_sim", "NaP_dist_count",
...
def summarize_metric_boxplot(root_dir, output_dir,
                             metric_name, file_prefix, suffixes, ylabel, showfliers=False):
    """
    ...
    for group, patterns in GROUPS.items():
        ...
        for pat in patterns:
            ...
            for suf in suffixes:
                ...
                fp = os.path.join(root_dir, f"IP_{pat}",
                                  "analyze_final_sim", "NaP_dist_count", fn)
                vals = load_values(fp)
                per_p.append(vals)
        ...
        n = len(patterns)
        fig, axes = plt.subplots(n, 1, figsize=(6, 1.5*n),
