...
def plot_dU_distributions_grid(base_dir1, base_dir2=None, offset_fix=0,
                               debug=False, no_normalize=False):
    """
    ...
    for i in range(n_plots):
        ...
        if i < n_fwd:
            ...
            if debug:
                print(f"[DEBUG] FWD i={i} λ={cLam_f:.4f} -> λ={nLam_f:.4f},"
        ...
        if i < n_bwd:
            ...
            if debug:
                print(f"[DEBUG] BWD i={i} λ={cLam_b:.4f} -> λ={nLam_b:.4f},"
    ...
    if no_normalize:
        ...
        ybox1 = TextArea(r"$P_{fwd}(\Delta U)$", textprops=dict(color="k", size=30, rotation=90, ha='center', va='bottom'))
        ybox2 = TextArea(", ", textprops=dict(color="k", size=30, rotation=90, ha='center', va='bottom'))
        ybox3 = TextArea(r"$P_{bwd}(\Delta U)$", textprops=dict(color="red", size=30, rotation=90, ha='center', va='bottom'))
        ybox = VPacker(children=[ybox3, ybox2, ybox1], align="center", pad=0, sep=2)
    else:
        ybox = TextArea("Scaled Density", textprops=dict(color="k", size=30, rotation=90, ha='center', va='bottom'))

    # Anchor the composite y-label to the figure.
    anchored_ybox = AnchoredOffsetbox(loc='center left', child=ybox, pad=0., frameon=False,
...
def main():
    ...
    parser.add_argument("base_dir1", 
                        help="First directory with subfolders 'lambda_*' (forward).")
