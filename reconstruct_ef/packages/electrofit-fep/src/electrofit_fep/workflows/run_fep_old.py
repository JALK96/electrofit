...
def set_up_fep_production(m_gro, MDP_dir, base_scratch_dir, molecule_name,
                          box_type="dodecahedron", cation="NA", anion="CL",
                          d="1.2", conc="0.15", lambda_values=None,
                          exit_screen=True):
    """
    ...
    try:
        ...
        for lam in lambda_values:
            ...
            os.mkdir(lam_dir)
            shutil.copy(os.path.join(scratch_dir, "npt.gro"), os.path.join(lam_dir, "conf.gro"))
            shutil.copy(m_top, lam_dir)
            logging.info(f"Running FEP simulation at lambda = {lam} in directory {lam_dir}")
            run_command(
