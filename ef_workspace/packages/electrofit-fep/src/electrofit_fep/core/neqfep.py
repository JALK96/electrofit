...
def compute_md_parameters(dt, nsteps, start_fraction=0.375, target_frames=80):
    """
    Compute simulation time, extraction start time, and output frequency parameters.

    Parameters:
    -----------
    dt : float
        Time step in picoseconds (ps). For example, 0.002 ps.
    nsteps : int
        Total number of simulation steps. For example, 3000000.
    start_fraction : float, optional
        Fraction of total simulation time after which snapshots extraction starts.
        Default is 0.375 (i.e. 37.5% of total time). For dt=0.002 and nsteps=3000000,
        total time = 6000 ps and start time = 6000 * 0.375 = 2250 ps.
    target_frames : int, optional
        Desired number of frames between start_time and end of simulation.
        Default is 80 frames.
    ...
    dict
        A dictionary with keys:
         ...
         - 'start_time': Extraction start time in ps.
         - 'nstxout': Output frequency (in simulation steps) for coordinates.
         - 'nstvout': Output frequency for velocities.
         - 'nstenergy': Output frequency for energies.

    Example:
    --------
    >>> params = compute_md_parameters(dt=0.002, nsteps=3000000)
    >>> print(params)
    {'total_time': 6000.0, 'start_time': 2250.0, 'nstxout': 23500, 'nstvout': 23500, 'nstenergy': 23500}
    """
    # Calculate total simulation time in ps
    total_time = dt * nsteps

    # Determine start time based on a fraction of total time.
    start_time = total_time * start_fraction

    # Calculate the available time window after the start_time.
    available_time = total_time - start_time

    # The time between frames (in ps) needed to generate target_frames in the available window.
    time_between_frames = available_time / target_frames

    # Convert the time between frames (ps) into simulation steps.
    output_interval = int(round(time_between_frames / dt))
...
class neqFEP:
    ...
    This workflow runs:
      ...
      3) Energy minimization, followed by two‐step equilibration:
           ...
           b) NPT equilibration (NPT_l0/NPT_l1)  <-- Extended longer to sample equilibrium
      4) Transitions: extraction of snapshots from the NPT run for free energy sampling
      5) Analysis of free energy differences
    ...
    mdpPath : str
        Directory holding .mdp files for different simulation phases:
          ...
          - NPT_l0.mdp, NPT_l1.mdp (second equilibration step, extended)
    ...
    def __init__(self, **kwargs):
        ...
        self.replicas = 3
        # Define simulation types (now omitting "prod").
    ...
    def prepareFreeEnergyDir(self):
        ...
        if self.ligandPath is None:
            raise ValueError(
                "ligandPath must be defined for ligand transformations in water."
            )
    ...
    def _get_specific_path(
        self, edge=None, bHybridStrTop=False, wp=None, state=None, r=None, sim=None
    ):
        if edge is None:
            ...
            for wp in self.thermCycleBranches:
                ...
                for state in self.states:
                    ...
                    for r in range(1, self.replicas + 1):
                        ...
                        # Create subfolders for each simulation type: em, nvt, npt, transitions
    ...
    def boxWaterIons(self, edges=None, bBox=True, bSolvate=True, bIon=True):
        ...
        for edge in edges:
            ...
            if bBox:
                print(
                    f"Call editconf to set up the box, with parameter: boxshape = {self.boxshape}, d = {self.boxd}"
                )
            ...
            # Solvate
            if bSolvate:
                print("Solvate: using cs='spc216.gro'")
    ...
    def prepare_simulation(self, edges=None, simType="em"):
        ...
        For non-equilibrium FEP we now use:
         ...
         - 'npt' for second equilibration (NPT, extended to collect snapshots),
         - 'transitions' for extracting snapshots from NPT.
        The input structure for each step comes from the previous step:
         - nvt uses output from em,
         - npt uses output from nvt,
         - transitions uses output from npt.
        """
        print("-----------------------------------------")
            ...
            for state in self.states:
                ...
                for r in range(1, self.replicas + 1):
                    ...
                    emp = (
                        None
                        if simType == "em"
                        else f"{outLigPath}/{state}/run{r}/{predecessor}"
                    )
    ...
    def _prepare_single_tpr(
        self, simpath, toppath, state, simType, empath=None, frameNum=0
    ):
        """
        ...
        if simType == "em":
            mdpFile = (
                f"{self.mdpPath}/em_l0.mdp"
                if state == "stateA"
                else f"{self.mdpPath}/em_l1.mdp"
            )
            inStruct = f"{toppath}/ions.pdb"
            outTPR = f"{simpath}/tpr.tpr"
        elif simType == "nvt":
            mdpFile = (
                f"{self.mdpPath}/NVT_l0.mdp"
                if state == "stateA"
                else f"{self.mdpPath}/NVT_l1.mdp"
            )
            inStruct = f"{empath}/confout.gro"
            outTPR = f"{simpath}/tpr.tpr"
        elif simType == "npt":
            mdpFile = (
                f"{self.mdpPath}/NPT_l0.mdp"
                if state == "stateA"
                else f"{self.mdpPath}/NPT_l1.mdp"
            )
            inStruct = f"{empath}/confout.gro"
            outTPR = f"{simpath}/tpr.tpr"
        elif simType == "transitions":
            mdpFile = (
                f"{self.mdpPath}/ti_l0.mdp"
                if state == "stateA"
                else f"{self.mdpPath}/ti_l1.mdp"
            )
        ...
        if not os.path.isfile(inStruct):
            msg = f"ERROR: Structure file not found: {inStruct}. Cannot prepare TPR for {simType}."
            raise FileNotFoundError(msg)

        print(
            f"Running grompp to prepare tpr from structure: {inStruct} using topology: {topFile} and mdp: {mdpFile}"
        )
        gmx.grompp(
            f=mdpFile,
            c=inStruct,
        ...
        for edge in edges:
            ...
            for state in self.states:
                ...
                for r in range(1, self.replicas + 1):
                    print(f"Run: {r}")
                    base_simpath = (
                        f"{self.workPath}/{edge}/water/{state}/run{r}/{simType}"
                    )
                    print("Base simulation path: ", base_simpath)
                    if simType == "transitions":
                        # For multiple frames (e.g. 80 frames)
                        for i in range(1, 81):
                            print("processig frame ...", i)
                        ...
                        self._run_mdrun(
                            tpr, ener, confout, mdlog, trr, bVerbose, xtc=xtc
                        )
                        self._clean_backup_files(simpath)
        print("DONE")
    ...
    def prepare_transitions(self, edges=None, bGenTpr=True, startTime=2250):
        """
        For non-equilibrium FEP, we now extract frames directly from the (extended) NPT run.
        This method extracts frames from the NPT run and prepares .tpr files for transitions.
        """
        print("---------------------")
        ...
        for edge in edges:
            ...
            for state in self.states:
                print(state)
                for r in range(1, self.replicas + 1):
                    # Use extended NPT output as input for transitions
                    eqpath = f"{outLigPath}/{state}/run{r}/npt"
    ...
    def run_analysis(self, edges=None, bVerbose=False):
        """
        Gathers (.xvg) files from stateA and stateB transitions,
        ...
        for edge in edges:
            ...
            for r in range(1, self.replicas + 1):
                ...
                self._run_analysis_script(
                    analysispath, stateApath, stateBpath, bVerbose
                )
        print("DONE")
    ...
    def _run_analysis_script(self, analysispath, stateApath, stateBpath, bVerbose):
        ...
        # -b Number of bootstrap samples to use for the bootstrap estimate of the standard errors. Default is 0 (no bootstrap).
    ...
    def _read_neq_results(self, fname):
        ...
        outvals = [
            ...
            0,
            0,
        ]  # [frames0->1, frames1->0, dG, err_analytical, err_bootstrap]
        for line in lines:
            line = line.strip()
            if "BAR: dG" in line:
                tokens = line.split()
