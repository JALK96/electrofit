        ...
        if parent_dir == current_dir:
            ...
            if root is None:
                raise FileNotFoundError(
                    f"Project root directory '{project_name}' not found."
                )
        ...
        if os.path.isdir(folder_path):
            ...
            for item in os.listdir(folder_path):
                ...
                if os.path.isfile(item_source_path):
                    ...
                    shutil.copytree(
                        item_source_path, item_dest_path, dirs_exist_ok=True
                    )
...
def rename_mol2_binary(base_dir, binary):
    ...
    if os.path.isdir(folder_path):
        ...
        print(
            f"Folder '{folder_path}' does not exist or does not match the binary name in mapping."
        )
