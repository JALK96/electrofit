...
from electrofit.io.files import find_file_with_extension, mol2_to_pdb_with_bonds
...
for sub_dir in os.listdir(process_dir):
    ...
    for i, c in enumerate(configs):
        ...
        mol2_to_pdb_with_bonds(
            input_file=input_mol2_file, existing_pdb_file=conform_path
        )

    print(
        f"Extracted conformations and bash script saved for {sub_dir} in {extracted_conforms_dir}"
    )
