...
while len(executed_scripts) < len(all_scripts):
    ...
    scripts_to_run = [
        script for script in all_scripts if script not in executed_scripts
    ][:batch_size]
    ...
    print(
        f"Executed {len(executed_scripts)} scripts so far. Waiting 10 minutes before running the next batch of {batch_size}."
    )
