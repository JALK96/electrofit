...
from alchemlyb.estimators import BAR, FEP_ESTIMATORS, MBAR, TI as _ALCHEMLYB_TI, TI_ESTIMATORS
...
# custimized TI class, if you want to use the alchemlyb TI class, you can use _ALCHEMLYB_TI instead.
    ...
    def separate_dhdl(self):
        ...
        for i in range(len(l_types)):
            if any(diff[:, i]):
                ...
                for line in l_types:
                    if line != l_types[i]:
                        new = new.reset_index(line, drop=True)
...
def plot_convergence(dataframe, units=None, final_error=None, ax=None):
    ...
    fp = FP(size=18)
    ax.legend(
        [line_forward[0], line_backward[0]],
        ["Forward", "Reverse"],
        loc="best",
        prop=fp,
        frameon=False,
    ...
    ax.set_xlabel(
        "Fraction of simulation time", color="k", fontsize=20
    )  # color="#151B54"
    ax.set_ylabel(
        r"$\Delta G$ ({})".format(units if units else ""), color="k", fontsize=20
    )
...
def parse_args():
    ...
    parser.add_argument("--skiptime", type=float, default=300, help="Skiptime in ps.")
    parser.add_argument(
        "--raw-only",
        action="store_true",
...
def main():
    ...
    if not file_list:
        raise ValueError(
            f"No files matched the pattern '{pattern}' under directory {dir_input}"
        )
    ...
    # =============================================================================
    logging.info(
        f"Preprocessing data (skiptime={skiptime}, uncorr={uncorr}, threshold={threshold}, raw_only={raw_only})."
    )
    u_nk_sample_list = []
    if u_nk_list:
        for index, df in enumerate(u_nk_list):
            ...
            if raw_only:
                logging.info(
                    f"State {index}: raw‑only flag set, skipping u_nk decorrelation."
                )
                ...
                if len(subsample) < threshold:
                    logging.warning(
                        f"u_nk for state {index} has only {len(subsample)} uncorrelated samples (< {threshold}). Using full data."
                    )
                    subsample = df_proc
                else:
                    logging.info(
                        f"State {index}: using {len(subsample)} uncorrelated u_nk samples."
                    )
    ...
    if dHdl_list:
        for index, df in enumerate(dHdl_list):
            ...
            if raw_only:
                logging.info(
                    f"State {index}: raw‑only flag set, skipping dHdl decorrelation."
                )
                ...
                if len(subsample) < threshold:
                    logging.warning(
                        f"dHdl for state {index} has only {len(subsample)} uncorrelated samples (< {threshold}). Using full data."
                    )
                    subsample = df_proc
                else:
                    logging.info(
                        f"State {index}: using {len(subsample)} uncorrelated dHdl samples."
                    )
    ...
    if u_nk_sample_list:
        ...
        logging.warning(
            "Using original u_nk data as decorrelated data is not available."
        )
    ...
    if dHdl_sample_list:
        ...
        logging.warning(
            "Using original dHdl data as decorrelated data is not available."
        )
    ...
    for estimator in estimators_to_use:
        if estimator in FEP_ESTIMATORS:
            ...
            if estimator == "MBAR":
                ...
                estimators_results["MBAR"] = MBAR(**MBAR_OPTS, n_bootstraps=10).fit(
                    u_nk_concat
                )
            elif estimator == "BAR":
    ...
    ref_estimator = estimators_results.get(
        "MBAR", next(iter(estimators_results.values()))
    )
    ...
    for i in range(num_states - 1):
        logging.info(
            f"  Transition {i}--{i + 1}: Lambda from {ref_estimator.states_[i]} to {ref_estimator.states_[i + 1]}"
        )
    ...
    ref_estimator_raw = estimators_results_raw.get(
        "MBAR", next(iter(estimators_results_raw.values()))
    )
    ...
    for i in range(num_states_raw - 1):
        logging.info(
            f"  Transition {i}--{i + 1}: Lambda from {ref_estimator_raw.states_[i]} to {ref_estimator_raw.states_[i + 1]}"
        )
    ...
    for estimator_name, estimator_obj in estimators_results.items():
        logging.info(
            f"Assembling results from estimator: {estimator_name} (decorrelated)"
        )
        ...
        for idx, stage in enumerate(stages):
            if len(stages) == 1:
                ...
                if lambda_min == lambda_max:
                    ...
                    start = (
                        num_states - list(reversed(state_values)).index(lambda_min) - 1
                    )
            ...
            if estimator_name != "BAR":
                ...
                error = np.sqrt(
                    sum(d_delta_f_.iloc[i, i + 1] ** 2 for i in range(start, end))
                )
    ...
    if not raw_only:
        ...
        if "MBAR" in estimators_results:
            ...
            ax_overlap = plot_mbar_overlap_matrix(
                estimators_results["MBAR"].overlap_matrix
            )
            ...
            logging.warning(
                "MBAR estimator not available for decorrelated data; skipping overlap plot."
            )
        ...
        if "TI" in estimators_results:
            ...
            logging.warning(
                "TI estimator not available for decorrelated data; skipping TI dH/dλ plot."
            )
        ...
        fig_df = plot_dF_state(
            estimators_results.values(),
            units=energy_units,
        ...
        plt.close(fig_df)
        logging.info(
            f"Free energy state differences (portrait, decorrelated) saved to {df_state_file}"
        )
        fig_df_land = plot_dF_state(
            estimators_results.values(),
            units=energy_units,
        ...
        logging.info(
            f"Free energy state differences (landscape, decorrelated) saved to {df_state_long_file}"
        )
        ...
        if "MBAR" in estimators_results:
            ...
            raise ValueError(
                "No suitable estimator available for decorrelated convergence analysis."
            )
        ...
        logging.info(
            f"Performing convergence analysis (decorrelated) with {conv_estimator}."
        )
        if conv_estimator == "MBAR":
            convergence_df = forward_backward_convergence(
                data_for_conv,
                estimator=conv_estimator,
                num=convergence_num_points,
                **MBAR_OPTS,
            )
        elif conv_estimator == "TI":
            convergence_df = forward_backward_convergence(
                data_for_conv, estimator=conv_estimator, num=convergence_num_points
            )
        else:
            raise ValueError(
                f"Estimator {conv_estimator} is not recognized for convergence analysis."
            )
    ...
    for estimator_name, estimator_obj in estimators_results_raw.items():
        ...
        for j in range(1, num_states_raw):
            ...
            summary_dict_raw[f"{estimator_name}_Error"].append(
                d_delta_f_.iloc[j - 1, j]
            )
        for idx, stage in enumerate(stages):
            if len(stages) == 1:
                ...
                if lambda_min == lambda_max:
                    ...
                    start = (
                        num_states_raw
                        - list(reversed(state_values)).index(lambda_min)
                        - 1
                    )
                    end = state_values.index(lambda_max)
            result = delta_f_.iloc[start, end]
        ...
        if estimator_name != "BAR":
            ...
            error_total = np.sqrt(
                sum(d_delta_f_.iloc[i, i + 1] ** 2 for i in range(num_states_raw - 1))
            )
    ...
    if "MBAR" in estimators_results_raw:
        ...
        ax_overlap_raw = plot_mbar_overlap_matrix(
            estimators_results_raw["MBAR"].overlap_matrix
        )
        ...
        logging.warning(
            "MBAR estimator not available for raw data; skipping overlap plot."
        )
    ...
    if "TI" in estimators_results_raw:
        ...
        logging.warning(
            "TI estimator not available for raw data; skipping TI dH/dλ plot."
        )

    logging.info("Plotting free energy state differences (raw data).")
    fig_df_raw = plot_dF_state(
        estimators_results_raw.values(),
        units=energy_units,
        orientation="portrait",
        nb=10,
    )
    ...
    logging.info(
        f"Free energy state differences (portrait, raw data) saved to {df_state_file_raw}"
    )
    fig_df_raw_land = plot_dF_state(
        estimators_results_raw.values(),
        units=energy_units,
        orientation="landscape",
        nb=10,
    ...
    logging.info(
        f"Free energy state differences (landscape, raw data) saved to {df_state_long_file_raw}"
    )
    ...
    if "MBAR" in estimators_results_raw:
        ...
        raise ValueError(
            "No suitable estimator available for raw convergence analysis."
        )
    ...
    logging.info(
        f"Performing convergence analysis (raw data) with {conv_estimator_raw}."
    )
    if conv_estimator_raw == "MBAR":
        convergence_df_raw = forward_backward_convergence(
            data_for_conv_raw,
            estimator=conv_estimator_raw,
            num=convergence_num_points,
            **MBAR_OPTS,
        )
    elif conv_estimator_raw == "TI":
        convergence_df_raw = forward_backward_convergence(
            data_for_conv_raw, estimator=conv_estimator_raw, num=convergence_num_points
        )
    else:
        raise ValueError(
            f"Estimator {conv_estimator_raw} is not recognized for convergence analysis."
        )
    logging.info("Convergence analysis results:")
