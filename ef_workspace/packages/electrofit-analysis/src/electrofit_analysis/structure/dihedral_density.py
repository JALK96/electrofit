            ...
            if os.path.isdir(run_final_sim_dir):
                ...
                for dihedral in dihedral_definitions:
                    ...
                    for name in dihedral:
                        ...
                        if len(sel) != 1:
                            raise ValueError(
                                f"Atom selection for name '{name}' did not return exactly one atom."
                            )
                ...
                dihedral_angles = np.array(
                    dihedral_angles
                )  # Shape: (num_frames, num_dihedrals)
                ...
                groups_to_plot_indices = [
                    i
                    for i, group in enumerate(dihedral_groups)
                    if group["name"].startswith("Dihedrals for C")
                ]
                num_groups = len(groups_to_plot)

                # Define a color map for however many groups we have
                colors_summary = plt.cm.get_cmap(cf.color_map)(
                    np.linspace(0, 1, len(groups_to_plot))
                )

                # Create one subplot per filtered group in one overall figure.
                fig, axes = plt.subplots(
                    nrows=num_groups, ncols=1, figsize=(5, 2 * num_groups), sharex=True
                )
                if num_groups == 1:
                    axes = [axes]
                ...
                for idx, group in enumerate(groups_to_plot):
                    ...
                    dihedral_atoms_shifted = [
                        shift_atom_number(atom) for atom in dihedral_atoms
                    ]
                ...
                for group_idx, group in enumerate(dihedral_groups):
                    ...
                    if group["name"].startswith("Dihedrals for C"):
                        ...
                        colors_carbon = plt.cm.get_cmap(cf.color_map)(
                            np.linspace(0, 1, 6)
                        )
                        for idx_in_group, dihedral_idx in enumerate(dihedral_idxs):
                            plt.plot(
                        ...
                        plt.title(
                            f"Dihedral Angles Over Time - {group['name']}", fontsize=16
                        )
                ...
                if len(carbon_data) > 0:
                    fig_carbon, ax_carbon = plt.subplots(
                        figsize=(5, 4), constrained_layout=True
                    )
                    colors_carbon = plt.cm.get_cmap(cf.color_map)(
                        np.linspace(0, 1, len(carbon_data))
                    )
                    ...
                    for i_idx, (d_idx, angles_deg_array, label) in reversed(
                        list(enumerate(carbon_data))
                    ):
                        # 1) Compute the histogram in memory
                        counts, bin_edges = np.histogram(
                            angles_deg_array, bins=360, range=(-180, 180), density=True
                        )
                        ...
                        linewidth = max(
                            linewidth, 1.0
                        )  # Ensure linewidth doesn't get too small
                    ...
                    ax_carbon.set_title(
                        "Carbon (Ring) Dihedral Hist with Vertical Offset", fontsize=16
                    )
                ...
                if len(carbon_data) > 0:
                    ...
                    for i_idx, (d_idx, angles_deg_array, label) in reversed(
                        ...
                        linewidth = max(
                            linewidth, 1.0
                        )  # Prevent the linewidth from becoming too thin
                ...
                if len(oxygen_data) > 0:
                    ...
                    for chunk_idx, chunk_data in enumerate(oxygen_chunks):
                        fig_oxy, ax_oxy = plt.subplots(
                            figsize=(7, 4), constrained_layout=True
                        )
                        colors_oxy = plt.cm.get_cmap(cf.color_map)(
                            np.linspace(0, 1, len(chunk_data))
                        )
                        ...
                        ax_oxy.set_title(
                            f"Oxygen Dihedral Hist Offset - Group {chunk_idx + 1}",
                            fontsize=16,
                        )
                        ...
                        outname = (
                            f"dihedral_hist_oxygen_group{chunk_idx + 1}_offset.pdf"
                        )
                        plt.savefig(outname)
                        plt.close()
                ...
                if len(oxygen_data) > 0:
                    ...
                    colors_summary = plt.cm.get_cmap(cf.color_map)(
                        np.linspace(0, 1, len(oxygen_chunks))
                    )
