...
def plot_results_from_csv(file_path, invert_TI=True, group_size=10):
    ...
    for i, (start, end) in enumerate(groups):
        ...
        ax.bar(x - bar_width, sub_df['MBAR'], bar_width,
               yerr=sub_df['MBAR_Error'], capsize=3, label='MBAR', color="#C45AEC")
        ax.bar(x + bar_width, sub_df['BAR'], bar_width,
               yerr=sub_df['BAR_Error'], capsize=3, label='BAR', color="#F9B7FF")
        ax.bar(x, sub_df['TI'], bar_width,
               yerr=sub_df['TI_Error'], capsize=3, label='TI', color="#6698FF")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Plot free energy summary from a CSV file in a given directory.")
    parser.add_argument("directory", help="Directory where the CSV file (free_energy_summary.csv) is located.")
    parser.add_argument("--csv", default="free_energy_summary.csv", help="Name of the CSV file (default: free_energy_summary.csv)")
    parser.add_argument("--invert-ti", action="store_true", help="Invert TI values (default: False)")
    parser.add_argument("--group-size", type=int, default=10, help="Number of lambda pairs per subplot (default: 10)")
    ...
    plot_results_from_csv(str(csv_file), invert_TI=invert_TI, group_size=args.group_size)
