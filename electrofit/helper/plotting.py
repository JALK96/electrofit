...
def plot_charges_by_atom_new_2(atoms_dict1, initial_charges_dict, base_dir, atoms_dict2=None):
    ...
    def set_violin_colors(vp, alpha_value):
        for i, body in enumerate(vp['bodies']):
            ...
            color = color_map.get(group_name, 'black')  # Default to black if group not in color_map
            # Set the face and edge color of the violin body
            body.set_facecolor(color)
            body.set_edgecolor(color)
            body.set_alpha(alpha_value)
    ...
    if charges_data2 is not None:
        handles.extend([
            plt.Line2D([], [], color='grey', alpha=0.7, marker='s', linestyle='None', markersize=10, label='Dataset 1'),
            plt.Line2D([], [], color='grey', alpha=0.4, marker='s', linestyle='None', label='Dataset 2'),
        ])
...
def plot_charges_by_atom_new_3(atoms_dict1, initial_charges_dict, base_dir, atoms_dict2=None):
    ...
    if charges_data2 is not None:
        ...
        ax.scatter(positions2, avg_charges2, color="red", marker="s", label="Average", zorder=5, s=7)

    # Add scatter points for initial charges
    ax.scatter(positions1, init_charges, color="black", marker='o', label="Initial Charge", zorder=5, s=7)
    ...
    for group_label, indices in groups.items():
        ...
        height = 0.1      # Height/depth of the brace; adjust as needed
    ...
    if charges_data2 is not None:
        handles.extend([
            plt.Line2D([], [], color='red', marker='s', linestyle='None', markersize=5, label='Combined Mean'),
            plt.Line2D([], [], color='grey', alpha=0.7, marker='s', linestyle='None', markersize=10, label='Dataset 1'),
            plt.Line2D([], [], color='grey', alpha=0.4, marker='s', linestyle='None', markersize=10, label='Dataset 2'),
        ])
...
def plot_charges_by_atom_new_4(atoms_dict1, initial_charges_dict, base_dir, atoms_dict2=None, equivalent_groups=None):
    ...
    if atoms_dict2 is not None:
        ...
        charges_data2_symmetric = [atoms_dict2[atom]['charges'] for atom in atom_names_symmetric]
        avg_charges2_symmetric = [atoms_dict2[atom]['average_charge'] for atom in atom_names_symmetric]
        avg_sum2 = sum(avg_charges2_symmetric)
    else:
    ...
    ax.scatter(non_symmetric_positions1, non_symmetric_avg_charges1, color="black", marker='s', label="Mean (Non-Symmetric)", zorder=5, s=7)

    # For symmetric atoms, plot their average charges from Dataset 2 at positions2_symmetric with red rectangles
    if charges_data2_symmetric is not None:
        ax.scatter(positions2_symmetric, avg_charges2_symmetric, color="red", marker='s', label="Combined Mean", zorder=5, s=7)
    ...
    handles.append(
        plt.Line2D([], [], color='black', marker='s', linestyle='None', markersize=5, label='Mean (Non-Sym)')
    )
    if charges_data2_symmetric is not None:
        handles.append(
            plt.Line2D([], [], color='red', marker='s', linestyle='None', markersize=5, label='Combined Mean')
        ...
        handles.extend([
            plt.Line2D([], [], color='grey', alpha=0.7, marker='s', linestyle='None', markersize=10, label='Sampled Charges'),
            plt.Line2D([], [], color='grey', alpha=0.4, marker='s', linestyle='None', markersize=10, label='Combined Charges'),
        ])
    else:
        handles.append(
            plt.Line2D([], [], color='grey', alpha=0.7, marker='s', linestyle='None', markersize=10, label='Dataset 1')
