...
def main():
    ...
    for run_dir in _iter_run_dirs(project_root):
        ...
        # Re-seed snapshot on every run (clean semantics). Upstream & other layers propagate each time.
        ...
        # Load the layered run-specific config (includes snapshot reseed just performed)
        ...
        # Allow per-run override of runtime threads/pin
        ...
        # Allow per-run override of base_scratch
        base_scratch_run = getattr(getattr(cfg_run, 'paths', None), 'base_scratch_dir', None) or "/tmp/electrofit_scratch"

        if not snap_path:
            logging.warning(f"[step3][warn] snapshot could not be created in {run_dir}")
        ...
        mdp_dir  = meta.get("mdp_dir") or (getattr(process_cfg.paths, "mdp_dir", None) or "MDP")
