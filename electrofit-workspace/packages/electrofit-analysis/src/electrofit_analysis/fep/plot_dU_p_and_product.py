...
def parse_dhdl_xvg_current_and_targets(dhdl_file, debug=False, skip_time=0.0):
    ...
    if debug:
        print(
            f"[DEBUG] parse_dhdl_xvg_current_and_targets({dhdl_file}) => "
...
def parse_single_subdir(dhdl_file, offset_fix=0, debug=False, skip_time=0.0):
    cLam, targets_map, time_arr, data_arr, legends_map = (
        parse_dhdl_xvg_current_and_targets(dhdl_file, debug=debug, skip_time=skip_time)
    )
    if cLam is None or data_arr.size == 0:
        return None, None, None, None
    nLam, col_idx = find_next_lambda_and_column(
        cLam, targets_map, data_arr.shape, offset_fix=offset_fix, debug=debug
    )
...
def gather_dU_per_subdir(base_dir, offset_fix=0, debug=False, skip_time=0.0):
    ...
    for idx, fpath in subdirs:
        ...
        if nLam is None or deltaU is None:
            print(
                f"[INFO] No next-lambda or no data for λ={cLam:.3f} in {fpath}, skip."
            )
            continue
...
def plot_3curves_for_each_transition(
    base_dir,
    offset_fix=0,
    debug=False,
    normalize=False,
    ...
    if normalize:
        ...
        ybox3 = TextArea(
            r"$P(\Delta U) \times \exp[-\beta \Delta U]$",
            textprops=dict(
                color="#6698FF", size=30, rotation=90, ha="center", va="bottom"
            ),
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Plot P(ΔU), exp(-βΔU), & product for each transition in base_dir. "
            "Also write out sigma and produce a sigma-vs-lambda plot."
        )
    )
    parser.add_argument(
        "base_dir",
        help="Directory with subfolders 'lambda_*' each containing dhdl.xvg.",
    )
    parser.add_argument(
        "--offset", type=int, default=0, help="Offset for sX->data column."
    )
    parser.add_argument(
        "--skip-time",
        type=float,
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Output filename for the plot. If not provided, "
    ...
    plot_3curves_for_each_transition(
        base_dir=base,
