...
def read_charges(chg_file_path):
    ...
    try:
        ...
        print(f"Error: Non-numeric value encountered in charges file '{chg_file_path}'.")
        print(f"Details: {ve}")
        sys.exit(1)
    return charges
...
def update_mol2_charges(mol2_input_path, chg_file_path, mol2_output_path):
    ...
    if num_charges != num_atoms:
        print(f"Error: Number of charges ({num_charges}) does not match number of atoms ({num_atoms}).")
        sys.exit(1)
    else:
    ...
    for i in range(num_atoms):
        ...
        print(f"Atom {i+1}: {atom_name} - Charge updated from {original_charge} to {charges[i]}")
    ...
    try:
        ...
        print(f"Error: Failed to rebuild MOL2 structure.")
        print(f"Details: {e}")
        sys.exit(1)
...
def main():
    ...
    if len(sys.argv) != 4:
        print("Usage: python update_mol2.py <input_MOL2_file> <chg_file> <output_MOL2_file>")
        print("Example: python update_mol2.py mol2_input.mol2 charges.chg fixed_out.mol2")
        sys.exit(1)
