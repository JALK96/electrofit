...
def compute_hbond_correlation_functions_optimized(
    ...
    for dt in range(max_lag_frames):
        if dt < n_frames:
            ...
            S_inter[dt] = np.sum(data_matrix[:, :valid_frames] * data_matrix[:, dt:dt+valid_frames])
    if count_origins > 0:
...
def main():
    ...
    parser.add_argument("xpm_file", type=str, help="Path to the XPM file (from gmx hbond -hbm).")
    parser.add_argument("--time_per_frame_ns", type=float, default=0.01,
                        help="Time step in ns for each frame. Default=0.01 ns (10 ps).")
    parser.add_argument("--max_lag_frames", type=int, default=1000,
                        help="Max number of frames for correlation time-lag. Default=1000.")
    parser.add_argument("--output_prefix", type=str, default="corr",
