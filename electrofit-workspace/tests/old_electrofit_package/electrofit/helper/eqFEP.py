...
class eqFEP:
    ...
    def __init__(self, **kwargs):
        ...
        self.lambdaStates = [0.00, 0.05, 0.10, 0.20, 0.30,
        ...
        for edge in edges:
            ...
            for state in self.states:
                for r in range(1, self.replicas+1):
                    ...
                    if simType == "em":
                        mdpFile = os.path.join(self.mdpPath, "em_l0.mdp") if state == "stateA" else os.path.join(self.mdpPath, "em_l1.mdp")
                    elif simType == "nvt":
                        mdpFile = os.path.join(self.mdpPath, "NVT_l0.mdp") if state == "stateA" else os.path.join(self.mdpPath, "NVT_l1.mdp")
                    elif simType == "npt":
                        mdpFile = os.path.join(self.mdpPath, "NPT_l0.mdp") if state == "stateA" else os.path.join(self.mdpPath, "NPT_l1.mdp")
    ...
    def _run_regular_sim(self, edges, simType, bVerbose):
        for edge in edges:
            for state in self.states:
                for r in range(1, self.replicas+1):
                    base_simpath = os.path.join(self.workPath, edge, "water", state, f"run{r}", simType)
    ...
    def prepare_transitions(self, edges=None, bGenTpr=True):
        ...
        """
        logging.info(f"Preparing multi-lambda transitions (bGenTpr={bGenTpr}). Cascade? {self.cascade}")
        if edges is None:
            edges = self.edges
        if self.cascade:
        ...
        for edge in edges:
            ...
            for state in self.states:
                ...
                for r in range(1, self.replicas + 1):
                    ...
                    npt_out = os.path.join(self.workPath, edge, "water", state, f"run{r}", "npt", "confout.gro")
                    ...
                    for i in range(len(self.lambdaStates)):
                        ...
                        if bGenTpr and not self.cascade:
                            self._prepare_single_lambda_tpr(lamFolder, mdpFile, npt_out,
    ...
    def _prepare_single_lambda_tpr(self, lamFolder, mdpFile, inStruct, toppath):
        logging.info(f"Preparing singele lamda tpr for folder: {lamFolder} with input structure: {inStruct} using topology: {toppath}")
        if not os.path.isfile(inStruct):
            raise FileNotFoundError(
        ...
        gmx.grompp(f=mdpFile, c=inStruct, p=topFile, o=outTPR,
                   maxwarn=4, other_flags=f" -po {mdout}")
            ...
            for state in self.states:
                for r in range(1, self.replicas+1):
                    ...
                    while iStart < numLams:
                        ...
                        with concurrent.futures.ThreadPoolExecutor(max_workers=self.windowsBatch) as executor:
                            future_map = {}
                            for cmd_str, lamFolder in commands:
                                future = executor.submit(run_command, cmd_str, lamFolder)
                            ...
                            for future in concurrent.futures.as_completed(future_map):
                                ...
                                try:
                                    ...
                                    logging.error(f"Error in lambda window {lamFolder}: {e}")
                                self._clean_backup_files(lamFolder)
    ...
    def _run_multi_lambda_cascade(self, edges, bVerbose=False):
        ...
        for edge in edges:
            for state in self.states:
                for r in range(1, self.replicas+1):
                    transBase = os.path.join(self.workPath, edge, "water", state, f"run{r}", "transitions")
                    input_structure = os.path.join(self.workPath, edge, "water", state, f"run{r}", "npt", "confout.gro")
                    ...
                    for i in range(len(self.lambdaStates)):
                        ...
                        if not os.path.isfile(mdpFile):
                            logging.warning(f"No MDP file found at {mdpFile}, creating one on the fly.")
                            # you'd fill in template logic here if skipping prepare_transitions
                        # Re-grompp using the current input structure
                        self._prepare_single_lambda_tpr(lamFolder, mdpFile, input_structure,
                                                        toppath=os.path.join(self.workPath, edge, "water"))
                        tpr = os.path.join(lamFolder, "ti.tpr")
                        ener = os.path.join(lamFolder, "ener.edr")
                        mdlog= os.path.join(lamFolder, "md.log")
                        ...
                        cmd_list = [
                            ...
                            "-s", tpr,
                            "-e", ener,
                            "-c", os.path.join(lamFolder, "confout.gro"), 
                            "-o", trr,
                            "-g", mdlog,
                            "-ntmpi", str(self.ntmpi),
                            "-ntomp", str(self.ntomp),
                            "-pin", "on",
                            "-pinoffset", "0"  # or could do something else
                        ]
                        if xtc:  cmd_list += ["-x", xtc]
                        if dhdl: cmd_list += ["-dhdl", dhdl]
                        cmd_str = " ".join(cmd_list)
                        logging.info(f"[CASCADE] Running window {i} with command: {cmd_str}")
                        try:
                        ...
                        if os.path.isfile(new_input):
                            ...
                            logging.error(f"Missing confout.gro from cascade step {i} in {lamFolder}!")
                            break
                        self._clean_backup_files(lamFolder)
    ...
    # -------------------------------------------------------
    def _run_mdrun(self, tpr, ener, confout, mdlog, trr, bVerbose=False, xtc=None, dhdl=None):
        cmd = [
            "gmx", "mdrun",
            "-s", tpr,
            "-e", ener,
        ...
        for edge in edges:
            for r in range(1, self.replicas + 1):
                for state in self.states:
                    ...
                    transitions_dir = os.path.join(self.workPath, edge, "water", state, f"run{r}", "transitions")
                    dhdl_files = []
                    for i in range(len(self.lambdaStates)):
                    ...
                    if not dhdl_files:
                        logging.warning(f"No dhdl.xvg files for replicate={r}, state={state}, skipping.")
                        continue
                    ...
                    try:
                        ...
                        logging.error(f"Error during BAR analysis for edge {edge}, replicate={r}, state={state}: {e}")
                        continue
                    analysis_dir = os.path.join(self.workPath, edge, "water", f"analyse_r{r}", state)
